...
namespace dotnetapp
{
    public class DataAccessLayer
    {
        ...
        public DataAccessLayer()
        {
            ...
            //SqlConnection con = new SqlConnection(@"Data Source = DESKTOP-F09N105; Initial Catalog = Bikeloan2 ;Integrated Security = true");
        }
        ...
        public List<LoanModel> getAllLoans()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    obj.applicantName = dt.Rows[0]["applicantName"].ToString();
                    obj.applicantMobile = dt.Rows[0]["applicantMobile"].ToString();
                    obj.applicantPan = dt.Rows[0]["applicantPan"].ToString();
                    obj.applicantAddress = dt.Rows[0]["applicantAddress"].ToString();
                    obj.loanId = Convert.ToInt32(dt.Rows[0]["loanId"]);
                    obj.applicantSalary = dt.Rows[0]["applicantSalary"].ToString();
                    obj.applicantEmail = dt.Rows[0]["applicantEmail"].ToString();
                    obj.applicantAadhaar = dt.Rows[0]["applicantAadhaar"].ToString();
        ...
        public string approveLoan(int loanId, string status)
        {
            ...
            try
            {
                ...
                if (i > 0)
                {
                    if (status == "Approved" && loanId == @loanId)
                    {
                        genSchedule(loanId);
                    }
        ...
        public string addUser(ProfileModel lm)
        {
            ...
            {
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = "usp_ProfileInsert2";
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters.AddWithValue("@email", lm.email);
                cmd.Parameters.AddWithValue("@emi", lm.emi);
                cmd.Parameters.AddWithValue("@loanId", lm.loanId);
                cmd.Connection = con;
                con.Open();
                int rowaffect = cmd.ExecuteNonQuery();
                con.Close();
        ...
        public LoanModel getLoan(int loanId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                lm.status = dt.Rows[0]["status"].ToString();
            }
            return lm;
        }
